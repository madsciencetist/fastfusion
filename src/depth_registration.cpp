             ...
             const image_geometry::PinholeCameraModel &rgb_model)
{
  ...
  DepthTraits<Tout>::initializeBuffer(registered_msg->data);
  ...
  const Tin *depth_row = reinterpret_cast<const Tin *>(&depth_msg->data[0]);
  int row_step = depth_msg->step / sizeof(Tin);
  Tout *registered_data = reinterpret_cast<Tout *>(&registered_msg->data[0]);
  int raw_index = 0;
  for (unsigned v = 0; v < depth_msg->height; ++v, depth_row += row_step)
  {
    for (unsigned u = 0; u < depth_msg->width; ++u, ++raw_index)
    {
      Tin raw_depth = depth_row[u];
      if (!DepthTraits<Tin>::valid(raw_depth))
        continue;
      ...
      Tout &reg_depth = registered_data[v_rgb * registered_msg->width + u_rgb];
      Tout new_depth = DepthTraits<Tout>::fromMeters(xyz_rgb.z());
      // Validity and Z-buffer checks
      if (!DepthTraits<Tout>::valid(reg_depth) || reg_depth > new_depth)
        reg_depth = new_depth;
    }
  }
}
                              ...
                              const tf2_ros::Buffer &tf_buffer)
{
  ...
  registered_msg->encoding = output_encoding;
